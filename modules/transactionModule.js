  ...
  async init(web3, tokenContract, poolConfig, accountConfig, redisInterface,
    ...
    */
    async batchMinedPayments() {
      ...
      console.log("PAY: " + unbatched_pmnts)
      if (unbatched_pmnts.length >= this.poolConfig.MIN_PAYMENTS_IN_BATCH) {
        ...
        var paymentsToBatch = unbatched_pmnts.slice(0, this.poolConfig.MAX_TO_BATCH)
    ...
    */
    ...
    async broadcastPaymentBatches() {
      ...
      try {
        ...
        for (var element of unconfirmedBatches) {
          ...
          if (element.broadcastedAt == null || element.broadcastedAt < (
              currentEthBlock - this.poolConfig.REBROADCAST_WAIT_BLOCKS)) {
      ...
      setTimeout(function() {
          self.broadcastPaymentBatches()
        }, this.poolConfig.BROADCAST_TIMEOUT) //ten minutes
...
console.log("GOT ONE TO SUBMIT")
    ...
    */
    async broadcastQueuedPaymentTransactions() {
      ...
      }, this.poolConfig.PAYMENT_BROADCAST_PERIOD)
    ...
    */
    ...
    async transferPaymentBatch(batchPayment) {
      ...
      console.log([tokenAddress, batchPayment.id, toAddressArray,
        toValueArray
      ])
    ...
    async transferTokensFromPool(addressFromType, ethMinerAddress, amount, balancePaymentId, resending) {
      ...
      var transferMethod = this.tokenContract.methods.transfer(addressTo,
        amount);
      ...
      if (addressFromType == 'minting') {
        ...
        var paymentConfirmed = await this.getBalanceTransferConfirmed(
          balancePaymentId);
